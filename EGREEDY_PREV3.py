...
class EGREEDY_PREV3(AbstractAlgo):

    def __init__(self):
        super(EGREEDY_PREV3).__init__()
    ...
    '''
    def egreedy(self, Q, n_teams, prev3, c, t):
    ...
    def rank_teams(self, n_teams, n_judges, true_q, c, var, top_n):
        ...
        while True: # WHILE q-values haven't converged
            ...
            winner = sim.judge(0,judge_current[j],judge_previous[j][2]) # simulate the judge's decision
            done = self.update(Q,N,judge_current[j],judge_previous[j][2],winner,t,top_n) # update the team's q-values
